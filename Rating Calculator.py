...
def take_individual_stat(analyzed):
    ...
    df =pd.read_csv(f"./Data/Data Set/{analyzed}.csv")
...
def atacking_stats(df):
    atacking_data = pd.read_csv('./Data/Data Set/Atacking Data.csv')
...
def rating_calculator(data,position):
    ...
    for player in players:
        rating = 3.1 # Lowest variation of rating
        for stat in stats:
...
def second_normalization(data,position):
    ...
    for player in players:
        ...
        ratings[player] = rating_deviation + goal_weight * ga_deviation + 3 # The 3 is so the lowest number is >0
    pd.DataFrame({'Name':ratings.keys(),'Rating':ratings.values()}).sort_values(by='Rating',ascending=False).to_csv(f'./Data/{position[1]}_{position[0]}_rating.csv')

 
second_normalization(atacking_stats(take_individual_stat(analyzed)),position_selector(analyzed))
